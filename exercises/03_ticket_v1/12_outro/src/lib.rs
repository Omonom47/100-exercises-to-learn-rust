...
fn verify_name(name: &String){
    if name.is_empty() {
        panic!("product_name may not be empty");
    }
    if name.len() > 300 {
        panic!("product_name must be at most 300 bytes")
    }
}
...
impl Order {
    pub fn new(product_name: String, quantity: u32, unit_price: u32) -> Order{
        verify_name(&product_name);
        verify_quantity(&quantity);
        verify_price(&unit_price);
        Order{
            product_name,
            quantity,
            unit_price
    ...
    pub fn set_quantity(&mut self, new_quantity: u32){
        verify_quantity(&new_quantity);
        self.quantity = new_quantity;
    }
    pub fn set_unit_price(&mut self, new_price: u32){
        verify_price(&new_price);
