...
impl TryFrom<String> for  TicketDescription {
    ...
    fn try_from(value: String) -> Result<Self, Self::Error> {
        if value.is_empty() {
            return Err(DescriptionParseError::EmptyDescription);
        }
        if value.len() > 500 {
            return Err(DescriptionParseError::TooLongDescription);
        }
        Ok(TicketDescription{0:value})
    }
}
impl TryFrom<&str> for  TicketDescription {
    ...
    fn try_from(value: &str) -> Result<Self, Self::Error> {
        ...
        Ok(TicketDescription{0:value.to_string()})
    }
}
